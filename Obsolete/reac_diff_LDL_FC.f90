...
program reac_diffuse
  ...
  DOUBLE PRECISION, ALLOCATABLE :: c_LDL_thresh_vec(:), R_LDL_rhs(:)
...
! Diffusion - reaction parameters
  ...
  ALLOCATE(c_LDL_thresh_vec(1:size(node_x,2)))
  ALLOCATE(R_LDL_rhs(1:size(node_x,2)))
  c_LDL_thresh_vec = 1.d0
  c_LDL_thresh_vec = c_LDL_thresh*c_LDL_thresh_vec
...
!  BC_nodes = (/1, 2, 3, 4/)
  ...
  if (modulo(n_s,1).eq.0) then
    LDL_strng = 'LDLconc'
    write(output_filename_LDL, '(A4, A7, I3, A4)') 'out/',LDL_strng, n_s,'.vtk'
    ...
    FC_strng = 'FCconc'
    write(output_filename_FC, '(A4, A6, I3, A4)'), 'out/',FC_strng, n_s,'.vtk'
...
!
 ...
 DEALLOCATE (c_LDL_thresh_vec,R_LDL_rhs,BC_toggle,BC_nodes )

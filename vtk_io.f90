...
subroutine vtk_puvw_write ( output_unit, title, node_num, element_num, &
  element_order, xyz, element_node, p )
...
!
  ...
  if ( element_order == 8 ) then
    do element = 1, element_num
      write ( output_unit, '(a)' ) '12'
    end do
  else if ( element_order == 27 ) then
    do element = 1, element_num
      write ( output_unit, '(a)' ) '27'
    end do
  end if

  write ( output_unit, '(a)' ) ' '
  write ( output_unit, '(a)' ) 'POINT_DATA ' // trim ( node_num_string )
  write ( output_unit, '(a)' ) 'SCALARS concentration double'
  write ( output_unit, '(a)' ) 'LOOKUP_TABLE default'
  do node = 1, node_num
    write ( output_unit, '(2x,ES16.7E4)' ) p(node)
  end do
  return
end
